...
visited <- read_csv("data/NP_visited.csv", col_names = TRUE, 
                    ...
                    col_types = list(
                      ...
                      lat = col_double(), 
                      lng = col_double(), 
                      type = col_factor(levels = c("np", "nhp", "nb", "nm", "nhs", "nmp", "nmem", "nhres", "mem", "park", "nbp", "nr", "nra", "ns", "mempkwy", "nha"))))
...
getColor <- function(visited){
  ...
  if(type == "np"){
    "green"
  }else if(type == "nhs" | type == "nhp" | type == "nhres" | type == "nha"){
    "orange"
      ...
      "purple"
    }else if(type == "nra" | type == "nr" | type == "ns"){
      "purple"
    }else{
    "yellow"
  } })
...
coloring <- data.frame(type = c("National Park", "Historic site", "Battlefield", "Memorial", "Recreation"), color = c("#6E8B3D", "#FF8C00", "#CD2626", "#009ACD", "#B23AEE"))

leaflet(visited) %>% 
  addTiles() %>% 
  addAwesomeMarkers(~lng, ~lat, popup = ~popup, label = ~popup, icon = icons, clusterOptions = markerClusterOptions()) %>% 
  addLegend("bottomright", colors = ~coloring$color, labels = ~coloring$type, opacity = 1)
```
